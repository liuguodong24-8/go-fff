...
contract BSCValidatorSet is IBSCValidatorSet, System, IParamSubscriber, IApplication {
    ...
    uint256 public constant maxStakeFFFCount = 100000 ether;

    uint256 public constant miniStakeFFFCount = 1 ether;

    uint256 public constant maxStakePeopleCount = 100000;

    uint256 public currStakePeople;

    uint256 public currStakePeopleIndex;

    uint256 public currStakeFFF;
  ...
  uint256 constant public DUSTY_INCOMING = 1e17;
    ...
    // will not transfer value less than 0.1 BNB for validators
  uint256 constant public MAX_STAKE_FFF_COUNT = 1111222333222111;
  ...
  uint256 public constant EXPIRE_TIME_SECOND_GAP = 1119;
  uint256 public constant MAX_NUM_OF_VALIDATORS = 41;

  bytes public constant INIT_VALIDATORSET_BYTES = hex"";
  ...
  mapping(address=>uint256) public stakeInfoMap;

  mapping(uint256=>address) public stakeInfoIndexMap;
  ...
  event feeBurned(uint256 amount);
    ...
    event UserStakeFFF(address user, uint256 count);

    event UserUnStakeFFF(address user, uint256 count);

    event DistributeBlockRewardFunc(uint256 userCount, uint256 count);
  /*********************** init **************************/
  ...
  function decodeValidator(RLPDecode.RLPItem memory itemValidator) internal pure returns(Validator memory, bool) {
    ...
    function StakeFFF() payable public{
        _StakeFFF(msg.sender,msg.value);
    }

    function _StakeFFF(address addr,uint256 count) private{

        uint256 stakeCount = count;
        require(stakeCount>=miniStakeFFFCount && stakeCount+stakeInfoMap[addr]<=maxStakeFFFCount ,"stake count must valid");
        require( (stakeInfoMap[addr]==0 && currStakePeople+1<=maxStakePeopleCount ) || stakeInfoMap[addr]!=0,"stake people out of range");
        if(stakeInfoMap[addr]==0) {
            stakeInfoIndexMap[currStakePeople]=addr;
            stakeInfoMap[addr]=stakeCount;
            currStakePeople++;
            currStakePeopleIndex++;
        }else{
            stakeInfoMap[addr]+=stakeCount;
        }
        currStakeFFF+=stakeCount;
        emit UserStakeFFF(addr,stakeCount);

    }


    function UnStakeFFF() payable public{
        address payable stakeAddress = payable(msg.sender);
        require(stakeInfoMap[msg.sender]>0,"must staked");
        uint256 stakeCount = stakeInfoMap[msg.sender];
        stakeAddress.transfer(stakeInfoMap[msg.sender]);
        currStakeFFF-=stakeInfoMap[msg.sender];
        stakeInfoMap[msg.sender]=0;
        currStakePeople--;
        emit UserUnStakeFFF(msg.sender,stakeCount);
    }

    function GetAllStakeInfo() public view  returns(StakeInfo[] memory){


       uint256 sumStake =0;
       for(uint256 i =0;i<currStakePeopleIndex;i++){
           address addr = stakeInfoIndexMap[i];
           uint256 count = stakeInfoMap[addr];
       ...
       StakeInfo[] memory addressList =new StakeInfo[](sumStake);
       ...
       for(uint256 i =0;i<currStakePeopleIndex;i++){
           ...
           if(count>0) {
            addressList[i].stakeAddress = addr;
            addressList[i].stakeCount = count;
           }
